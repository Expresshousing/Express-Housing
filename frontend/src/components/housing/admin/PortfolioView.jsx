          ...
          return (
            <section key={building.id} aria-labelledby={`building-${building.id}`}>
              <div className="grid gap-6 md:grid-cols-[220px_1fr] md:items-end">
                ...
                <div className="border-b pb-5" style={{ borderColor: c.BORDER }}><div><p className="text-[10px] font-bold uppercase tracking-[0.2em]" style={{ color: c.BLUE }}>{visibleUnits.length} of {units.length} units shown</p><h3 id={`building-${building.id}`} className="mt-3 text-[30px] font-extrabold leading-none md:text-[38px]">{building.name}</h3><p className="mt-3 text-[12px]" style={{ color: c.MUTED }}>{building.address}, Philadelphia</p></div></div>
              </div>
