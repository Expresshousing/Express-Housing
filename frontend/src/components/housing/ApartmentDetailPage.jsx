    ...
    >
      ...
      <div className="relative left-1/2 h-[58svh] max-h-[780px] min-h-[430px] w-[100dvw] max-w-none -translate-x-1/2 overflow-hidden sm:h-[70svh] sm:min-h-[520px]" style={{ background: "#0A0A0A" }}>
        ...
        {apt.images?.length > 1 && (
          <button
            type="button"
            onClick={() => openGallery(mainImg)}
            className="absolute bottom-4 right-4 z-10 inline-flex min-h-12 items-center justify-center rounded-full bg-white px-6 text-[13px] font-bold text-black shadow-[0_8px_24px_rgba(0,0,0,0.24)] transition-transform hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-black active:scale-[0.97] md:bottom-6 md:right-6"
      ...
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-x-16 lg:gap-y-0">
        ...
        <aside className="lg:col-span-5 lg:col-start-8 lg:row-span-2 lg:row-start-1 lg:self-stretch"><div className="rounded-[20px] border p-5 lg:sticky lg:top-24 lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto lg:p-7" style={{ background: c.CARD, borderColor: c.BORDER, boxShadow: isDarkMode ? "none" : "0 8px 30px rgba(0,0,0,0.08)" }}>
          ...
          <div className="mt-3 flex flex-wrap items-baseline gap-3"><strong className="text-[30px] font-extrabold">${Math.round(apt.nightly_rate)}<span className="text-[13px] font-medium" style={{ color: c.MUTED }}> / night</span></strong></div><p className="mt-1.5 text-[11px] leading-relaxed" style={{ color: c.MUTED }}>Stays of 30 nights or longer are charged at a lower nightly rate. Taxes and cleaning appear before request.</p>
          ...
          <div className="mt-6 space-y-4" data-testid="booking-panel">
            ...
            <label className="flex min-h-11 cursor-pointer items-start gap-3 text-[13px]" style={{ color: c.TEXT }}>
              <span className="relative mt-1 inline-flex h-[14px] w-[14px] shrink-0">
                <input type="checkbox" className="absolute inset-0 h-full w-full cursor-pointer opacity-0" checked={parkingRequested} onChange={(event) => setParkingRequested(event.target.checked)} />
                <span aria-hidden="true" className="pointer-events-none flex h-[14px] w-[14px] items-center justify-center rounded-[4px] border" style={{ borderColor: parkingRequested ? c.BLUE : c.BORDER, background: parkingRequested ? c.BLUE : c.INPUT_BG }}>
                  {parkingRequested && <Check size={10} color="#FFFFFF" strokeWidth={3} />}
                </span>
              </span>
              <span>Request parking <span className="block text-[11px]" style={{ color: c.MUTED }}>${(apt.parking_monthly || 0).toLocaleString()}/month. Short-stay parking is confirmed separately.</span></span>
            </label>
